...
const EditParkModal: React.FC<EditParkModalProps> = ({ isOpen, onClose, park, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(
                `${process.env.REACT_APP_API_URL}parks/park/${park.id}`,
                {
                    method: "PUT",
            ...
            if (!response.ok) {
                ...
                throw new Error(err.message || "Failed to update park");
            }
            ...
            alert(`Error: ${err.message || "Failed to update park"}`);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden">
                ...
                <div className="px-6 py-4 bg-blue-50 border-b border-blue-200">
                    <div className="flex items-center gap-4">
                        {park?.image && (
                            <div className="h-16 w-16 overflow-hidden rounded-lg bg-gray-200 flex-shrink-0">
                                <img
                                    src={park.image}
                                    alt={park.name}
                        ...
                        <div className="flex-1">
                            <p className="text-sm text-gray-600">Park ID</p>
                            <p className="text-lg font-bold text-gray-800">#{park?.id}</p>
                            <p className="text-sm text-gray-600 mt-1">
                                {park?.Bike?.length || 0} motorbikes parked
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="px-6 py-6 space-y-5 max-h-[60vh] overflow-y-auto">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition"
                                required
                            />
                        </div>
                        ...
                        <div>
                            <label className="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-700">
                                <MdImage className="h-4 w-4 text-blue-600" />
                                Image URL (Optional)
                            </label>
                            <input
                                type="text"
                    ...
                    <div className="flex items-center justify-end gap-3 border-t border-gray-200 bg-gray-50 px-6 py-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed transition shadow-md hover:shadow-lg"
                        >
